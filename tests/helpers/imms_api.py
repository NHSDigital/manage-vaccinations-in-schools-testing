...
class imms_api_helper:
    ...
    def _search_get_with_params(self, params: dict):
        return requests.get(endpoint=ImmsEndpoints.SEARCH, header=self.header(), param=params)

    def _search_post_with_payload(self, payload: dict):
        return requests.post(endpoint=ImmsEndpoints.SEARCH, header=self.header(), payload=payload, param=None)
    ...
    def search_with_both_methods(self, params: dict):
        ...
        assert _json1 == _json2, f"Responses from param search and payload search do not match {_json1} and {_json2}."
