...
@pytest.fixture
def url_with_mmrv_session_scheduled(schedule_mmrv_session_and_get_consent_url, schools):
    yield from schedule_mmrv_session_and_get_consent_url(
        schools[Programme.MMRV.group][0],
        Programme.MMRV,
    )


@pytest.fixture
def setup_session_for_mmrv(setup_session_and_batches_with_fixed_child):
    return setup_session_and_batches_with_fixed_child(Programme.MMRV)


def test_recording_mmrv_vaccination_e2e_with_triage(
    url_with_mmrv_session_scheduled,
    setup_session_for_mmrv,
    page,
    schools,
    children,
    nurse,
    ...
    """
    Test: End-to-end test for recording an MMRV vaccination for a child.
    Steps:
    ...
    3. Agree to MMRV vaccination, fill address, answer health questions, and confirm.
    4. Log in as nurse, navigate to session, set session in progress,
       register child as attending, triage (set as safe to vaccinate).
    5. Record MMRV vaccination for the child.
    Verification:
    - Final consent message is shown after online consent.
    - Vaccination is recorded for the child in the session.
    """
    child = children[Programme.MMRV][0]
    schools = schools[Programme.MMRV]
    mmrv_batch_name = setup_session_for_mmrv[Vaccine.MMR_VAXPRO]
    number_of_health_questions = len(
        Programme.health_questions(Programme.MMRV, ConsentOption.MMR_EITHER)
    )

    OnlineConsentWizardPage(page).go_to_url(url_with_mmrv_session_scheduled)
    ...
    # Triage step added for MMRV
    SessionsSearchPage(page).click_session_for_programme_group(
        schools[0], Programme.MMRV
    )
    SessionsOverviewPage(page).tabs.click_children_tab()
    SessionsChildrenPage(page).search.search_and_click_child(child)
    SessionsPatientPage(page).click_programme_tab(Programme.MMRV)
    ...
    vaccination_record = VaccinationRecord(child, Programme.MMRV, mmrv_batch_name)
...
def test_verify_child_cannot_be_vaccinated_twice_for_mmrv_on_same_day(
    url_with_mmrv_session_scheduled,
    setup_session_for_mmrv,
    page,
    schools,
    ...
    mmrv_batch_name = setup_session_for_mmrv[Vaccine.MMR_VAXPRO]
    number_of_health_questions = len(
        Programme.health_questions(Programme.MMRV, ConsentOption.MMR_WITHOUT_GELATINE)
    )
...
def test_recording_mmrv_vaccination_e2e_with_imported_dose_one(
    url_with_mmrv_session_scheduled,
    setup_session_for_mmrv,
    page,
    schools,
    ...
    """
    Test: End-to-end test for recording a second MMRV vaccination for a child with a
    previously imported dose 1.
    Steps:
    1. Import a historical vaccination file containing MMRV dose 1 for the child.
    ...
    4. Consent to MMRV vaccination, fill address, answer health questions, and confirm.
    5. Log in as nurse, navigate to session, set session in progress,
       register child as attending, triage (set as safe to vaccinate).
    6. Record MMRV vaccination for the child (dose 2).
    Verification:
    - Final consent message is shown after online consent.
    - Vaccination is recorded for the child in the session.
    """
    ...
    # Import vaccination file with MMRV dose 1
    list(upload_offline_vaccination(Programme.MMRV))
    LogInPage(page).log_out()
