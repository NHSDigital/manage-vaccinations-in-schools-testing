...
class pg_consent_doubles:
    ...
    def fill_child_name_details(
        ...
        self.po.act(locator=self.TXT_CHILD_FIRST_NAME, action=actions.FILL, value=child_first_name)
        self.po.act(locator=self.TXT_CHILD_LAST_NAME, action=actions.FILL, value=child_last_name)
        if known_as_first == test_data_values.EMPTY and known_as_last == test_data_values.EMPTY:
            self.po.act(locator=self.RDO_KNOWN_BY_ANOTHER_NAME_NO, action=actions.RADIO_BUTTON_SELECT)
        else:
            self.po.act(locator=self.RDO_KNOWN_BY_ANOTHER_NAME_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_KNOWN_AS_FIRST, action=actions.FILL, value=known_as_first)
            self.po.act(locator=self.TXT_KNOWN_AS_LAST, action=actions.FILL, value=known_as_last)
    ...
    def select_child_school(self, scenario_id: str, school_name: str) -> None:
        if school_name == self.po.get_element_property(
            ...
            self.po.act(locator=self.RDO_SELECT_SCHOOL, action=actions.RADIO_BUTTON_SELECT)
        else:
            self.po.act(locator=self.RDO_SELECT_DIFFERENT_SCHOOL, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
            self.po.act(locator=self.TXT_SCHOOL_NAME, action=actions.SELECT_FROM_LIST, value=school_name)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)

    def fill_parent_details(self, scenario_id: str, parent_name: str, relation: str, email: str, phone: str) -> None:
        self.po.act(locator=self.TXT_PARENT_FULL_NAME, action=actions.FILL, value=parent_name)
        ...
        if phone != test_data_values.EMPTY:
            self.po.act(locator=self.TXT_PHONE_OPTIONAL, action=actions.FILL, value=phone)
    ...
    def select_consent_for_vaccination(self, scenario_id: str, consent_for: str) -> None:
        match consent_for.lower():
            case "both":
                self.po.act(locator=self.RDO_CONSENT_BOTH, action=actions.RADIO_BUTTON_SELECT)
            case "menacwy":
                self.po.act(locator=self.RDO_CONSENT_ONE, action=actions.RADIO_BUTTON_SELECT)
                self.po.act(locator=self.RDO_CONSENT_MENACWY, action=actions.RADIO_BUTTON_SELECT)
            case "td/ipv":
                self.po.act(locator=self.RDO_CONSENT_ONE, action=actions.RADIO_BUTTON_SELECT)
                self.po.act(locator=self.RDO_CONSENT_TDIPV, action=actions.RADIO_BUTTON_SELECT)
            case _:
                self.po.act(locator=self.RDO_CONSENT_NONE, action=actions.RADIO_BUTTON_SELECT)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)

    def fill_gp_details(self, scenario_id: str, gp_name: str = test_data_values.EMPTY) -> None:
        if gp_name == test_data_values.EMPTY:
            self.po.act(locator=self.RDO_GP_NOT_KNOWN, action=actions.RADIO_BUTTON_SELECT)
        else:
            self.po.act(locator=self.RDO_GP_REGISTERED, action=actions.RADIO_BUTTON_SELECT)
    ...
    def fill_address_details(self, scenario_id: str, line1: str, line2: str, city: str, postcode: str) -> None:
        ...
        self.po.act(locator=self.TXT_ADDRESS_POSTCODE, action=actions.FILL, value=postcode)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def select_bleeding_disorder(
        ...
        if bleeding_disorder_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=bleeding_disorder_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def select_severe_allergies(self, scenario_id: str, allergy_details: str = test_data_values.EMPTY) -> None:
        if allergy_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=allergy_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def select_severe_reaction(self, scenario_id: str, reaction_details: str = test_data_values.EMPTY) -> None:
        if reaction_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=reaction_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def select_extra_support(self, scenario_id: str, extra_support_details: str = test_data_values.EMPTY) -> None:
        if extra_support_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=extra_support_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def vaccinated_in_past(self, scenario_id: str, vaccs_in_past_details: str = test_data_values.EMPTY) -> None:
        if vaccs_in_past_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=vaccs_in_past_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def other_vaccs_in_past(self, scenario_id: str, other_vaccs_in_past_details: str = test_data_values.EMPTY) -> None:
        if other_vaccs_in_past_details == test_data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=other_vaccs_in_past_details)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def verify_final_message(self, scenario_id: str, expected_message: str) -> None:
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value=expected_message)
    ...
    def select_consent_not_given_reason(self, scenario_id: str, reason: str, reason_details: str) -> None:
        match reason.lower():
            case self.VACCINE_ALREADY_RECEIVED:
                self.po.act(locator=self.RDO_VACCINE_ALREADY_RECEIVED, action=actions.RADIO_BUTTON_SELECT)
                self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
                self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=reason_details)
            case self.VACCINE_WILL_BE_GIVEN_ELSEWHERE:
                self.po.act(locator=self.RDO_VACCINE_WILL_BE_GIVEN_ELSEWHERE, action=actions.RADIO_BUTTON_SELECT)
                self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
                self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=reason_details)
            case self.MEDICAL_REASONS:
                self.po.act(locator=self.RDO_VACCINE_MEDICAL_REASONS, action=actions.RADIO_BUTTON_SELECT)
                self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
                self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=reason_details)
    ...
    def service_give_consent(self):
        ...
        self.po.act(locator=self.RDO_ONLINE, action=actions.RADIO_BUTTON_SELECT)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def service_refuse_consent(self):
        ...
        self.po.act(locator=self.RDO_VACCINE_ALREADY_RECEIVED, action=actions.RADIO_BUTTON_SELECT)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
        self.po.act(locator=self.TXT_GIVE_DETAILS, action=actions.FILL, value=self.VACCINE_WILL_BE_GIVEN_ELSEWHERE)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
        self.po.act(locator=self.BTN_CONFIRM, action=actions.CLICK_BUTTON)
        self.po.verify(
    ...
    def parent_2_verbal_refuse_consent(self):
        self.po.act(locator=self.RDO_PARENT2_MUM, action=actions.RADIO_BUTTON_SELECT)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)  # Parent contact details page
        self.po.act(locator=self.RDO_IN_PERSON, action=actions.RADIO_BUTTON_SELECT)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def change_parent_phone(self):  # MAVIS-1778
        self.po.act(locator=self.TXT_PHONE_OPTIONAL, action=actions.FILL, value="7700900000")
        self.po.act(locator=self.CHK_TEXT_ALERTS, action=actions.CHECKBOX_CHECK)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
    ...
    def parent_1_online_positive(self):
        ...
        self.po.act(locator=self.CHK_TEXT_UPDATES, action=actions.CHECKBOX_CHECK)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)  # Online consent
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)  # Yes they agree
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)  # Health questions again
        self.po.act(locator=self.BTN_CONTINUE, action=actions.CLICK_BUTTON)  # Safe to vaccinate
        self.po.act(locator=self.BTN_CONFIRM, action=actions.CLICK_BUTTON)
    ...
    def update_triage_outcome_positive(self):
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Triage outcome updated for")
