...
class pg_parental_consent:
    ...
    def select_consent_for_vaccination(self, scenario_id: str, consented: bool = True) -> None:
        if consented:
            self.po.act(locator=self.CHK_CONSENT_AGREE, action=element_actions.CHECKBOX_CHECK)
        else:
            self.po.act(locator=self.CHK_CONSENT_DISAGREE, action=element_actions.CHECKBOX_CHECK)
        self.po.act(locator=self.BTN_CONTINUE, action=element_actions.CLICK_BUTTON)
    ...
    def select_medical_condition(self, scenario_id: str, medical_condition_details: str = data_values.EMPTY) -> None:
        if medical_condition_details == data_values.EMPTY:
            ...
            self.po.act(locator=self.RDO_YES, action=element_actions.RADIO_BUTTON_SELECT)
            self.po.act(locator=self.TXT_GIVE_DETAILS, action=element_actions.FILL, value=medical_condition_details)
        self.po.act(locator=self.BTN_CONTINUE, action=element_actions.CLICK_BUTTON)
