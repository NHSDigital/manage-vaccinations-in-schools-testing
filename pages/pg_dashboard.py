...
class pg_dashboard:
    ...
    def verify_all_expected_links_for_nurse(self):
        self.po.verify(locator=self.LNK_PROGRAMMES, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(locator=self.LNK_SESSIONS, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(locator=self.LNK_CHILDREN, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(locator=self.LNK_VACCINES, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(
            locator=self.LNK_IMPORT_RECORDS, property=properties.VISIBILITY, expected_value=True, exact=True
        )
        self.po.verify(
        ...
        self.po.verify(locator=self.LNK_SCHOOL_MOVES, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(locator=self.LNK_ORGANISATION, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(
            locator=self.LNK_SERVICE_GUIDANCE, property=properties.VISIBILITY, expected_value=True, exact=True
        )
        self.po.verify(
            locator=self.LNK_IMPORT_RECORDS, property=properties.VISIBILITY, expected_value=True, exact=True
        )
    ...
    def verify_all_expected_links_for_superuser(self):
        ...
        self.po.verify(locator=self.LNK_SCHOOL_MOVES, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(
            locator=self.LNK_NOTICES, property=properties.VISIBILITY, expected_value=True, exact=True
        )  # Superuser only
        self.po.verify(locator=self.LNK_ORGANISATION, property=properties.VISIBILITY, expected_value=True, exact=True)
        self.po.verify(
            locator=self.LNK_IMPORT_RECORDS, property=properties.VISIBILITY, expected_value=True, exact=True
