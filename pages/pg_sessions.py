...
class pg_sessions:
    ...
    def click_child_no_consent(self):
        self.po.act(locator=self.TXT_FILTER_NAME, action=actions.FILL, value=self.LNK_CHILD_NO_CONSENT)
        self.po.act(locator=None, action=actions.WAIT, value=wait_time.MIN)
    ...
    def download_offline_recording_excel(self) -> str:
        ...
        self.po.act(locator=self.LNK_DOWNLOAD_EXCEL, action=actions.DOWNLOAD_FILE_USING_LINK, value=_file_path)
        return _file_path
    ...
    def __set_gillick_competence(self, is_add: bool, is_competent: bool, competence_details: str) -> None:
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value=competence_details, exact=False)
    ...
    def __schedule_session(self, on_date: str, expect_error: bool = False):
        ...
        if expect_error:
            ...
            self.po.verify(
                locator=self.LBL_MAIN, property=properties.TEXT, expected_value=_expected_message, exact=False
            )
    ...
    def __delete_sessions(self):
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value=self.LBL_NO_SESSIONS_SCHEDULED)
    ...
    def __close_session(self):
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Session closed.")
    ...
    def invalidate_parent2_refusal(self):
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent refusedInvalid")
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Invalidation notes.")
        self.po.act(locator=self.LNK_BACK, action=actions.CLICK_LINK)
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent refusedInvalid")
        self.po.verify(
            locator=self.LBL_MAIN,
    ...
    def verify_scheduled_date(self, message: str):
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value=message, exact=False)
    ...
    def bug_mavis_1696(self):
        ...
        self.po.verify(
            locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent from Parent2 invalidated"
        )
        self.po.verify(
            locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent refused by Parent2 (Mum)"
        )
        self.po.verify(
            locator=self.LBL_MAIN,
            property=properties.TEXT,
    ...
    def bug_mavis_1864(self):
        ...
        self.select_consent_refused()
        self.click_child_consent_twice()
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Dad refused to give consent.")
        self.click_activity_log()
        self.po.verify(
            locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent refused by Parent1 (Dad)"
        )
        self.po.verify(
            locator=self.LBL_MAIN,
            property=properties.TEXT,
        ...
        self.po.verify(
            locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent given by Parent1 (Dad)"
        )
    ...
    def verify_attendance_filters(self):
        ...
        self.po.act(locator=self.BTN_UPDATE_RESULTS, action=actions.CLICK_BUTTON)
        self.po.act(locator=None, action=actions.WAIT, value=wait_time.MIN)
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="No children")
        ...
        if len(self.ce.child_list) >= 1:
            for child_name in self.ce.child_list:
                ...
                self.po.act(locator=None, action=actions.WAIT, value=wait_time.MIN)
                self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value=child_name)
    ...
    def bug_mavis_1818(self):
        ...
        self.po.verify(locator=self.LBL_MAIN, property=properties.TEXT, expected_value="Consent given")
        self.click_activity_log()
        self.po.verify(
            locator=self.LBL_MAIN,
            property=properties.TEXT,
    ...
    def _answer_menacwy_prescreening_questions(self, check_prefilled: bool = False):
        ...
        if check_prefilled:
            self.po.verify(
                locator=self.CHK_ARE_FEELING_WELL, property=properties.CHECKBOX_CHECKED, expected_value=True
            )
        else:
            self.po.act(locator=self.CHK_ARE_FEELING_WELL, action=actions.CHECKBOX_CHECK)
